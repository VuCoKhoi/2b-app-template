import { flattenDeep, groupBy, isEqual, pick, uniq } from "lodash";
  ...
  async createProduct(
    ...
    const product = await this.shopifyClientRepository.createProduct(
      this.shopifyClient,
      {
        ...
        images: productDataConverted.images.map((img) => ({ src: img })),
    ...
    await Promise.all(
      inventoryItemIds.map((invId) =>
        this.shopifyClientRepository.updateInventory(this.shopifyClient, {
          id: invId,
